...
dic = pickle.load(open("/atlas_scratch/jschroder/Calibration_AR4_AR5/2ndrun/Plots/RCP60-Sresab1.p", 'rb'))
def make_CD(dic):
	...
	list_data = [Scatter(x=df.index , y = df[model], line = Line(color=coldict[model],width=1.5,dash=dash[model]),mode='lines',text=model,name=model,hoverinfo='y+text') for model in models]
	...
	f = open('/home/UA/jschroder/cd_ratio_comp_rcp60_sresa1b.html', 'w')
	f.write(html_final)
	f.close()
...
def make_TAB(dic):
	...
	html_final = html_start + plot_html + html_end
	f = open('/home/UA/jschroder/TAB_rcp60_sresa1b.html', 'w')
	f.write(html_final)
	f.close()
