...
def delta_production(list_of_list):
	...
	if scenario == 'sresa2':
		path2 = os.path.join( path,'AR4_CMIP3_models')
	else : 
		path2 = os.path.join( path,'AR5_CMIP5_models')

	df_mod = create_list(os.path.join( path2,scenario,model,variable))
...
if __name__ == '__main__':
	...
	scenarios = ['sresa2','rcp45','rcp85']

	path = '/Data/Base_Data/Climate/AK_CAN_2km/projected/'
	output_base = '/workspace/Shared/Users/jschroder/LCC/Deltas_tiff'
	baseline_input = '/home/UA/jschroder/historical_AkCAN_Prism_cropped'

	if not os.path.exists( output_base  ):
		os.mkdir( output_base )

	attribute_list = [item for item in itertools.product([scenarios[0]],CMIP4_models,variables)] + [item for item in itertools.product(scenarios[1:],CMIP5_models,variables)]
